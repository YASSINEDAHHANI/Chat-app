...
function Requirements() {
  ...
  // ADD: User role state to track permissions
  ...
  // UPDATED: Fetch user role along with project data
  useEffect(() => {
    const fetchProjectData = async () => {
      try {
        // First check user session to get role
        const sessionResponse = await api.get('/check_session');
        if (sessionResponse.data.logged_in) {
          setUserRole(sessionResponse.data.role || 'user');
        }
  ...
  // Icon components (keeping all your existing icon components)
  const PlusIcon = () => (
  ...
  return (
    <div style={styles.container}>
      <header style={styles.header}>
        <div style={styles.headerContainer}>
          ...
          <div style={styles.headerActions}>
            {/* LLM Settings Button - Only for managers and admins */}
            {(userRole === 'manager' || userRole === 'admin') && (
              <button
                style={{
                  ...styles.secondaryButton,
                  ...(hoveredItem === "settings-btn" ? styles.secondaryButtonHover : {}),
                }}
                onClick={() => setIsSettingsModalOpen(true)}
                onMouseEnter={() => setHoveredItem("settings-btn")}
                onMouseLeave={() => setHoveredItem(null)}
                title="Configurer les paramètres du modèle LLM"
              >
      ...
      {/* UPDATED: LLM Settings Modal - Only for managers and admins */}
      {(userRole === 'manager' || userRole === 'admin') && (
        <LLMSettingsModal
